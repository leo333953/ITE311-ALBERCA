            ...
            <div class="modal-body">
                <form id="editUserForm">
                    ...
                    <div class="mb-3" id="roleSelectContainer">
                        ...
                        <div id="roleRestrictionMessage" class="text-muted small mt-1" style="display: none;">
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                // If user is admin, disable role selection and show message
                if (response.user.role === 'admin') {
                    $('#editRole').prop('disabled', true);
                    $('#roleRestrictionMessage').show();
                } else {
                    $('#editRole').prop('disabled', false);
                    $('#roleRestrictionMessage').hide();
                }
...
function updateUser() {
    ...
    const isRoleDisabled = $('#editRole').prop('disabled');
    ...
    // Only include role if it's not disabled (not admin user)
    if (!isRoleDisabled) {
        formData.role = role;
    }
